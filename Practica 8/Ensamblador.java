...
public class Ensamblador{
	...
	public void compilar(){
		...
		Vector<String> relativosPendientes = new Vector<String>();
		BufferedWriter bwi=null;
		BufferedWriter bwe=null;
		try{
			//br es el archivo de entrada
			br = new BufferedReader(new InputStreamReader(new FileInputStream(rutaArchivo+".asm"), "utf-8"));
			String linea;
			...
			bwi = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(rutaArchivo+".tmp"),"utf-8"));
				...
				if(tokens!=null && tokens.length==3){
					...
					switch(tokens[1]){
						...
						case "DS.B": case "DS": case "RMB":
							...
							if(argumento<0 || argumento>=65535){
								errores=true;
								bwe.write("Línea "+lin+(argumento==65537?". El argumento no es un numero bien formado\n":". Argumento fuera de rango\n"));
							}else{
								if(!errorEtiqueta){
						...
						default:
							...
							if(!Clasificador.esEtiqueta(tokens[2]))
							...
							if(modos.startsWith("Error:")){
								...
								if(modo.startsWith("Error:")){
									...
									//Si son relativos verifica que no hayan desbordamientos. 
									if(modo.equals("REL8")){
										if(Clasificador.esEtiqueta(tokens[2])){
											relativosPendientes.add(""+lin+",REL8,"+contLoc+","+tokens[2]);
										}else{
											Integer rr=Clasificador.parseInt(tokens[2])-contLoc;
											if(rr<-128 || rr>127){
												errores=true;
												bwe.write("linea "+lin+"\t"+": Desbortamiento en modo REL8\n");
											}
										}
									}else if(modo.equals("REL16")){
										if(Clasificador.esEtiqueta(tokens[2])){
											//System.out.println(linea);
											relativosPendientes.add(""+lin+",REL16,"+contLoc+","+tokens[2]);
										}else{
											Integer rr=Clasificador.parseInt(tokens[2])-contLoc;
											if(rr<-32768 || rr>32767){
												errores=true;
												bwe.write("linea "+lin+"\t"+": Desbortamiento en modo REL16\n");
											}
										}
									}
								}
			...
			//System.out.println(relativosPendientes.size());
			for(int i=0;i<relativosPendientes.size();i++){
				//System.out.println(relativosPendientes.elementAt(i));
				StringTokenizer strtok = new StringTokenizer(relativosPendientes.elementAt(i),",");
				linea= strtok.nextToken();
				String modo = strtok.nextToken();
				int pc = Integer.parseInt(strtok.nextToken());
				String et = strtok.nextToken();
				if(!tabsim.existe(et)){
					//System.out.println(et+" no existe");
				}else{
					int arg=tabsim.valor(et);
					int rr=arg-pc;
					if(modo.equals("REL8")){
						if(rr<-128 || rr>127){
							errores=true;
							bwe.write("linea "+linea+"\t"+": Desbortamiento en modo REL8\n");
						}
					}else{
						if(rr<-32767 || rr>32768){
							errores=true;
							bwe.write("linea "+linea+"\t"+": Desbortamiento en modo REL16\n");
						}
					}
	...
	public void parse(){
		try{
			BufferedReader arch=new BufferedReader(new InputStreamReader(new FileInputStream(rutaArchivo+".tmp"), "utf-8"));
			BufferedWriter salida=BufferedWriter(new OutputStreamWriter(new FileOutputStream(rutaArchivo+".ins"),"utf-8"));
			String linea;
			while((linea=arch.readLine())!=null){
				...
				switch(modo){
					...
					case "REL16":
						...
						rr = Clasificador.parseInt(operando)-pc;
						if(rr<0){
							rr+=65536;
						}
