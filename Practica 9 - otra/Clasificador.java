...
public class Clasificador{
	...
	public static String[] procesar(String s){
		...
		while( idx<aux.length() && aux.charAt(idx)!=' ' && aux.charAt(idx)!='\t'){
			tok1=tok1+Character.toUpperCase(aux.charAt(idx));
			idx++;
		}
		...
		switch(tokens.size()){
			...
			case 3:
				...
				if(Clasificador.esEtiqueta(tokens.get(0))){
					if(!s.toUpperCase().startsWith(tokens.get(0).toUpperCase())){
						ret = new String[1];
...
/**
	...
	/**
	 *Convierte una cadena a codigo maquina
	 *@param String s, la cadena a ser convertida
	 *@return Una cadena hexadecimal con el codigo maquina de la cadena
	 */
	public static String strToHex(String s){
		String ret ="";
		for(int i=0;i<s.length();i++){
			ret+=intHexByte(s.charAt(i));
		}
		return ret;
	}
